...
\section{Heterogeneous Sorting Centre Environments}
...
On this level, \glspl{infoblock} are generated. This is a Swiss Post internal term used for report-ready data products. An infoblock consists of cleaned, deduplicated, curated data featuring joined datasets that use business naming, e.g. \gls{postday} and processing priority types. Basically, the infoblocks are the final layer employed in the reporting layer, making them accessible to reporting tools such as \gls{DSP} and \gls{SAC}, which are described in detail below. A more common term for an infoblock known throughout the industry is \textit{data product} \cite{alation_data_product, ibm_data_product}.
...
\subsubsection*{SAP Datasphere (DSP) and SAP Analytics Cloud (SAC)}
