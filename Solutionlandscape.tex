...
At the start of this project, the objective was to develop a minimal viable product (\gls{MVP}) to replace a manual data quality check performed by the business with a more robust solution, including automation, the delivery of reports, as well as and appropriate notification and alerting. However, after beginning this project, Swiss Post management assigned a new priority to the topic of data quality management (\gls{DQM}) overall; therefore, quite unexpectedly, this project acquired an additional set of key stakeholders, located in different teams as discussed in chapter \ref{sec:AWSmodernisationteam}. 
...
The current architectural goal followed by the \gls{DQM} team is shown in the following visualisation. The rule evaluation engine, seen top middle runs defined rules on target data and then stores the rule results in a database. In this context, a rule represents a check for data quality, or more precisely for a defined dataset; many different rules can exist. These standards or rules could, for example, consist of allowed values, such as gender allowing only three strings: male, female, and not stated. Alternatively, they could define allowed value types, for example permitting only dates in UTC format, or expected value ranges depending on weekdays.
...
Out of the complete target solution architecture, the first part to be implemented is. A rule-based DQM suite aimed to define generic checks that can be implemented across different datasets. This can be achieved by abstracting the check from the data, or by providing an SQL query that delivers the data in a form that allows the same check to be applied to different datasets. This approach leads to four main steps:
...
This initiative is of high importance to the project, as it creates a major organisation-wide constraint to follow the lead of the DQM team regarding their tool choice. The reasoning for developing a rule-based DQM solution lies in the fact that existing tools, such as \gls{AWSGlueDQ}, do not allow views with complex data types, or that \gls{PyDeequ} does not allow row-level results. These limitations in the evaluated tools led to the decision to build a dedicated solution at Swiss Post, as described above. Here is a short overview of the tools and approaches that were evaluated but then discarded as unsuitable.
...
\subsection*{Different sorting process}
...
The data relevant to this project are those used by the DWH LS OPS team. They consist of cleaned and deduplicated data stored in a \gls{curated} database layer. However, these data are derived from sensor data and arrive at the data warehouse (\gls{DWH}) in a raw format. The data pass through four main steps, which are also shown in the following figure:

\begin{itemize}
    \item \textbf{Physical data source}: Scans are performed on parcels by different scanners in sorting centers. As described above the Swiss Post uses a variety of different devices and especially hardware vendors, and that naturally leads to a very large number of data entities that must be stored.  Although researching the actual number was not required, it can be estimated that over X data entities flow from the scanners into the DWH.
...
However, a single visualisation does not explain the full benefit of the intended solution. For instance, the question arises as to why the business should stop performing data quality checks when it has the most detailed knowledge of the data. This is quickly clarified by taking a step back. The question should not be why checks are removed from the business, but why the reports are performed by the business in the first place. The benefits of this shift are therefore outlined in more detail in the following.

\subsection*{Benefit 1: Automation over manual processes}
...
\subsubsection*{Benefit 2: Scalability}
...
\subsection*{Benefit 3: Separation of concerns}
...
\subsubsection*{Benefit 4: Reusability across use cases}
